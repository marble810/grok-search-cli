...
# 或显式指定版本标签
# ./start-local-test-session.ps1 -Version v1.0.0

# 下面是等价的分步命令：
pwsh ./package-local-release.ps1 -Version v1.0.0
$LocalInstallDir = Join-Path $env:TEMP "grok-search-cli-local\bin"
pwsh ./install.ps1 -Version v1.0.0 -AssetDir ./artifacts/local-release -InstallDir $LocalInstallDir
$env:Path = "$LocalInstallDir;$env:Path"
```

macOS/Linux:

```bash
./package-local-release.sh --version v1.0.0
LOCAL_INSTALL_DIR="/tmp/grok-search-cli-local/bin"
./install.sh --version v1.0.0 --asset-dir ./artifacts/local-release --dir "$LOCAL_INSTALL_DIR"
export PATH="$LOCAL_INSTALL_DIR:$PATH"
```

完整的本地 install -> auth -> uninstall 测试步骤见 [01-installation-and-auth.md](01-installation-and-auth.md) 中的“1.0 纯本地生命周期冒烟”。PowerShell 可直接运行 `./start-local-test-session.ps1` 完成 temp 安装并把 CLI 注入当前会话 PATH。

**方式 B：从源码直接构建（仅检查编译产物）**
