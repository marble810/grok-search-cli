...
4. Add docs and release/install handoff text that points users from installation into `grok-search-cli auth login`.

   Release/install handoff guidance:

   ```
   After installing grok-search-cli, configure your xAI API key by running:

       grok-search-cli auth login

   This will store the key in your user configuration directory
   (~/.config/grok-search-cli/credentials.env on Linux/macOS,
   %APPDATA%/grok-search-cli/credentials.env on Windows).

   For automation (CI, agents, scripts), pipe the key via stdin:

       echo "$XAI_API_KEY" | grok-search-cli auth login --api-key-stdin

   The CLI also accepts XAI_API_KEY directly from the environment or a
   project-level .env file. The resolution order is:
     1. XAI_API_KEY environment variable (highest priority)
     2. .env file found by walking upward from the current directory
     3. Managed credential store (lowest priority, written by auth login)
   ```
